...
const nextPosition = duck => prev => {
  ...
  return {
    left: prev.right && duck.x >= duck.maxX,
    right: prev.left && duck.x <= 0,
...
class Duck extends Component {
  ...
  tick = () => {
    this.setState(nextPosition(this.props))
...
class App extends Component {
  ...
  shoot = id => {
    ...
    this.setState(prev => ({
      ducks: prev.ducks.map((duck, i) => (id == i ? { shot: true } : duck)),
    }))
    setTimeout(
      () =>
        this.setState(prev => ({
          ducks: prev.ducks.map((duck, i) => (id == i ? { dead: true } : duck)),
        })),
      1000,
    )
  }
